...
for currTime in range(p.simTime):
    ...
    if p.eMBMS_triggerTime != -1 and not currTime % (p.mcchModificationPeriod * 10) - p.eMBMS_triggerTime:
        ...
        if p.decFlag and p.rateEmbmsRs:  # decreasing the resource for eMBMS
            modResourceAlloSchemeforeMBMS(-0.01, currTime)
        elif p.incFlag:  # increasing the resource for eMBMS
            modResourceAlloSchemeforeMBMS(0.01, currTime)
        else:
